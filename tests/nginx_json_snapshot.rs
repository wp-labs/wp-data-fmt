...
fn json_string_escape() {
    ...
    let record = DataRecord {
        id: Default::default(),
        items: vec![
            FieldStorage::from_owned(DataField::from_chars("msg", "He said \"hi\"")),
...
fn json_null_and_float_precision() {
    let record = DataRecord {
        id: Default::default(),
        items: vec![
            FieldStorage::from_owned(DataField::new(DataType::Auto, "maybe", Value::Null)),
            FieldStorage::from_owned(DataField::from_float("pi", std::f64::consts::PI)),
        ],
    };
